...
abstract class ImageUtils {
    companion object {
        ...
        // For gray scale images with one channel
        fun bitmapToByteBufferGray(
            bitmapIn: Bitmap,
            width: Int,
            height: Int,
            mean: Float = 0.0f,
            ...
            val inputImage = ByteBuffer.allocateDirect(1 * width * height * 1 * 4)
            inputImage.order(ByteOrder.nativeOrder())
            inputImage.rewind()
            ...
            for (y in 0 until height) {
                for (x in 0 until width) {
                    ...
                    //inputImage.putFloat(((value shr 16 and 0xFF) - mean) / std)  // RED
                    //inputImage.putFloat(((value shr 8 and 0xFF) - mean) / std)   // GREEN
                    inputImage.putFloat(((value and 0xFF) - mean) / std)     // BLUE
                }
            }
